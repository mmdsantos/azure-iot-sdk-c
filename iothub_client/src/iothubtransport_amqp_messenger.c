...
static void destroy_event_sender(MESSENGER_INSTANCE* instance)
{
	if (instance->message_sender != NULL)
	{
		...
		instance->message_receiver_current_state = MESSAGE_SENDER_STATE_IDLE;
		instance->message_receiver_previous_state = MESSAGE_SENDER_STATE_IDLE;
...
{ 
	if (context != NULL)
	{
		...
		{
			LogInfo("messenger on_event_send_complete_callback invoked for timed out event %d; not firing upper layer callback.", task->message);
		}
