...
static void on_event_send_complete_messenger_callback(IOTHUB_MESSAGE_LIST* iothub_message, MESSENGER_EVENT_SEND_COMPLETE_RESULT ev_send_comp_result, void* context)
{
	...
	else
	{
		...
		// Codes_SRS_DEVICE_09_064: [The user callback and context saved in `send_task` shall be invoked passing the device `event_send_complete`]
...
static MESSENGER_DISPOSITION_RESULT on_messenger_message_received_callback(IOTHUB_MESSAGE_HANDLE iothub_message_handle, void* context)
{
	...
	else
	{
		...
		// Codes_SRS_DEVICE_09_075: [Any memory allocated by device_subscribe_message shall released]
		free(device_context);
	}
	...
	else
	{
		...
		if (instance->state == DEVICE_STATE_STARTING)
		{
			...
			if (instance->config->authentication_mode == DEVICE_AUTH_MODE_X509 || instance->auth_state == AUTHENTICATION_STATE_STARTED)
			{
				...
				if (instance->msgr_state == MESSENGER_STATE_STOPPED)
				{
					// Codes_SRS_DEVICE_09_042: [If messenger_start fails, the device state shall be updated to DEVICE_STATE_ERROR_AUTH]
...
int device_send_event_async(DEVICE_HANDLE handle, IOTHUB_MESSAGE_LIST* message, ON_DEVICE_D2C_EVENT_SEND_COMPLETE on_device_d2c_event_send_complete_callback, void* context)
{
	...
	// Codes_SRS_DEVICE_09_051: [If `handle`, `message` or `on_device_d2c_event_send_complete_callback` is NULL, device_send_event_async shall return a non-zero result]
	if (handle == NULL || message == NULL || on_device_d2c_event_send_complete_callback == NULL)
	{
		LogError("Failed sending event (either handle (%p), message (%p) or callback (%p) is NULL)", 
			handle, message, on_device_d2c_event_send_complete_callback);
		result = __LINE__;
	}
	else
	{
...
int device_subscribe_message(DEVICE_HANDLE handle, ON_DEVICE_C2D_MESSAGE_RECEIVED on_message_received_callback, void* context)
{
	...
	else
	{
		...
		else
		{
			...
			if (messenger_subscribe_for_messages(instance->messenger_handle, on_messenger_message_received_callback, device_context) != RESULT_OK)
			{
...
int device_set_option(DEVICE_HANDLE handle, const char* name, void* value)
{
	...
	else
	{
			...
			strcmp(DEVICE_OPTION_SAS_TOKEN_LIFETIME_SECS, name) == 0)
		{
			...
			if (instance->authentication_handle == NULL)
			{
				LogError("failed setting device option (cannot set authentication option '%s'; not using CBS authentication)", name);
				result = __LINE__;
			}
			...
			else if(authentication_set_option(instance->authentication_handle, name, value) != RESULT_OK)
			{
				...
				LogError("failed setting device option (failed setting authentication option '%s')", name);
				result = __LINE__;
			}
			else
			{
		...
		else if (strcmp(DEVICE_OPTION_EVENT_SEND_TIMEOUT_SECS, name) == 0)
		{
			...
			if (messenger_set_option(instance->messenger_handle, name, value) != RESULT_OK)
			{
				...
				LogError("failed setting device option (failed setting messenger option '%s')", name);
				result = __LINE__;
			}
			else
			{
		...
		else if (strcmp(DEVICE_OPTION_SAVED_AUTH_OPTIONS, name) == 0)
		{
			...
			else if (OptionHandler_FeedOptions((OPTIONHANDLER_HANDLE)value, instance->authentication_handle) != OPTIONHANDLER_OK)
			{
				...
				LogError("failed setting device option (OptionHandler_FeedOptions failed for authentication instance)");
				result = __LINE__;
			}
			else
			{
		...
		else if (strcmp(DEVICE_OPTION_SAVED_MESSENGER_OPTIONS, name) == 0)
		{
			...
			{
				// Codes_SRS_DEVICE_09_091: [If any call to OptionHandler_FeedOptions fails, device_set_option shall return a non-zero result]
				LogError("failed setting device option (OptionHandler_FeedOptions failed for messenger instance)");
				result = __LINE__;
			}
			else
			{
		...
		else if (strcmp(DEVICE_OPTION_SAVED_OPTIONS, name) == 0)
		{
			...
			{
				// Codes_SRS_DEVICE_09_091: [If any call to OptionHandler_FeedOptions fails, device_set_option shall return a non-zero result]
				LogError("device_set_option failed (OptionHandler_FeedOptions failed)");
				result = __LINE__;
			}
			else
			{
		...
		else
		{
			...
			LogError("device_set_option failed (option with name '%s' is not suppported)", name);
			result = __LINE__;
		}
	}
	...
	else
	{
		...
		else
		{
				...
				(dependency_options = authentication_retrieve_options(instance->authentication_handle)) == NULL)
			{
				...
				LogError("Failed to retrieve options from device instance (failed to retrieve options from authentication instance)");
				result = NULL;
			}
			// Codes_SRS_DEVICE_09_098: [The authentication options shall be added to `options` using OptionHandler_AddOption as DEVICE_OPTION_SAVED_AUTH_OPTIONS]
				...
				OptionHandler_AddOption(options, DEVICE_OPTION_SAVED_AUTH_OPTIONS, (void*)dependency_options) != OPTIONHANDLER_OK)
			{
				// Codes_SRS_DEVICE_09_102: [If any call to OptionHandler_AddOption fails, device_retrieve_options shall return NULL]
				LogError("Failed to retrieve options from device instance (OptionHandler_AddOption failed for option '%s')", DEVICE_OPTION_SAVED_AUTH_OPTIONS);
				result = NULL;
			}
			...
			else if ((dependency_options = messenger_retrieve_options(instance->messenger_handle)) == NULL)
			{
				...
				LogError("Failed to retrieve options from device instance (failed to retrieve options from messenger instance)");
				result = NULL;
			}
			...
			else if (OptionHandler_AddOption(options, DEVICE_OPTION_SAVED_MESSENGER_OPTIONS, (void*)&dependency_options) != OPTIONHANDLER_OK)
			{
				// Codes_SRS_DEVICE_09_102: [If any call to OptionHandler_AddOption fails, device_retrieve_options shall return NULL]
				LogError("Failed to retrieve options from device instance (OptionHandler_AddOption failed for option '%s')", DEVICE_OPTION_SAVED_MESSENGER_OPTIONS);
				result = NULL;
			}
			else
			{
