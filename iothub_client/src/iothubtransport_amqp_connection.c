...
static int create_cbs_handle(AMQP_CONNECTION_INSTANCE* instance)
{
	...
	/* TODO: provide 'on_cbs_state_changed' to cbs_create when cbs state changes become useful. 
	*        as of now, when cbs_destroy is called this callback is invoked with AMQP_MANAGEMENT_STATE_ERROR, which is incorrect. Bug? Yes.
	*/
